...
operates without modifying any gesture thresholds or application logic.
The system is publicly available at
\url{https://air-conductor.vercel.app/}; a demonstration video is at
\url{https://youtu.be/KhBOaTDZv-4}.
\end{abstract}
...
\begin{enumerate}
  ...
  interactive elements without altering any gesture threshold or
  application logic.
\end{enumerate}
...
Air Conductor departs from this paradigm. Rather than requiring the
user to memorise a gesture-to-pitch mapping, it places the user in the
role of conductor: all pitch decisions are delegated to the loaded MIDI
score, and the user controls only temporal and dynamic properties.
This design is further distinguished by articulation-level gestures
(fermata, cut-off) that operate directly on the audio engine's internal
state, a capability absent from discrete-trigger playing systems.
...
Table~\ref{tab:gestures} summarises all four gestures.

\begin{table*}[b!]
  \caption{Four-Gesture Vocabulary}
  \label{tab:gestures}
  \centering
  \begin{tabular}{p{2.2cm}p{1.5cm}p{2.8cm}p{10.0cm}}
    \hline
    \textbf{Gesture} & \textbf{Hand} & \textbf{Sensor} & \textbf{Effect} \\
    \hline
    Fingertip speed & Right & Pose lm[20] & Modulates playback tempo
      (EWMA, $\alpha=0.35$) \\
    Finger height & Left  & Pose lm[19,11,23] & Adjusts note velocity
      ($\pm 0.01$/frame; full range in $\approx$3.2\,s at 60\,fps) \\
    Fist          & Left  & Hand lm & Cut-off: stops all notes
      (300\,ms hold; 800\,ms resume) \\
    Pinch         & Left  & Hand lm & Fermata: sustains current chord
      (20\,ms hold) \\
    \hline
  \end{tabular}
\end{table*}
...
(\texttt{hudCanvas}), an overlay that shares dimensions with the camera
feed canvas, driven by a third \texttt{requestAnimationFrame} loop
fully decoupled from both the audio and vision loops. It shows:
...
Existing browser-based ADMIs implement a \emph{playing} paradigm in
which the user triggers specific pitches or chords via discrete
gestures. Air Conductor implements a \emph{conducting} paradigm in
which the score handles all pitch decisions and the user controls only
temporal and dynamic properties. Removing pitch selection from the
interaction loop reduces the working-memory demand of the interface and
supports engagement across a wider range of musical experience and
cognitive ability levels. The conducting model further allows users with
any level of musical training to participate meaningfully: a conductor
need not know individual note names, only how to express faster, slower,
louder, and softer.
...
scales the interface without modifying any gesture logic or thresholds.
...
\bibitem{rw13}
P. Adenot and C. Wilson,
``Web Audio API,'' W3C Recommendation,
World Wide Web Consortium, Dec. 2021. [Online]. Available:
