...
\noindent A pinch held for $\geq 20\,\text{ms}$ while the scheduler is
playing induces a fermata; the \texttt{scheduler.playing} guard
prevents accidental activation during the pre-start waiting period,
where an inadvertent pinch would otherwise invoke
\texttt{scheduler.resume()} and start playback prematurely.
The entry sequence is: (1) \texttt{scheduler.pauseOnly()}---which
freezes score advancement without invoking \texttt{releaseAll()}, so
currently-sounding notes continue to decay through their natural release
envelopes; (2) identify the current chord from
\texttt{scheduler.beatSnapshots}---a list of per-beat note sets appended
by \texttt{\_scheduleBeat} at pre-scheduling time, each tagged with its
scheduled audio fire time $t_{\text{beat}}$---by selecting the most
recent snapshot whose $t_{\text{beat}} \leq t_{\text{now}}$. Indexing
by beat rather than by individual note avoids two hazards of a
note-level map: false exclusion of common-tone notes whose entries are
overwritten by the 150\,ms lookahead pre-scheduling of the next beat,
and false inclusion of previous-beat notes through an overly wide
staleness window;
